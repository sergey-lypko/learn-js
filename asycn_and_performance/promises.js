...
// при заказе бургера на кассе создается своего рода обещание, что он (заказ) будет выполнен
...
// промисы позволяют работать с асинхронным кодом в синхронном виде
...
// промисы являются прекрасно подходящим для повторного использования механизмом, 
// который позволяет инкапсулировать и выстроить в композиция будущие значения (future values)


// * * Completion Event * *

// допустим есть некая foo(), которая может быть выполнена немедленно либо ее выполнение может занять некоторое время
// нам только необходимо знать, когда foo() завершится, чтобы мы могли продолжить со следующим task
// другими словами, мы хотели бы быть уведомлены (notified) о выполнении этой функции, чтобы затем продолжить
...
event.on("completion", () => { // do some })
event.on("failure", (err) => { // handle error })
  
// видно, что foo вообще не имеет отношения к дальнейшим обработчикам, которые будут слушать событие
// это разделение ответственности 
  
// важно, что здесь происходит обратная инверсия контроля (как это было с callback обработчиками) - 
// теперь вместо того, чтобы передавать callback'и в foo, функция возвращает некий event,
// на который можно подписатся и -> реагировать 
  
// разделение ответственности крайне полезно и позволяет совершенно  
// независимо обрабатывать различные сценарии
