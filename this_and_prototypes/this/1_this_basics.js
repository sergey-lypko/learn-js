// Первое, что стоит уяснить - this динамичен.
...
// this по сути никак не связывается с тем местом, где функция создавалась,
// но полностью полагается на то, где она вызывается и при каких обстоятельствах

// Во время вызова функции создается контекст выполнения,
// который содержит такую информацию как: откуда функция вызывалась (call-site),
// как функция была выполнена, какие параметры принимала и тд. Контекст выполнения
// также будет содержать ссылку this, которая будет испльзваться напротяжении
