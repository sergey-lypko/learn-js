...
// в этом случае ни создание нового свойства, не перекрытие недоступны (для всех объектов
// ниже по цепочке)
var ob = {}
...
// для возможности изменения значение свойства у объектов ниже по цеочке, 
// необходимо переопределить дескриптор
var ob = {
  a: 1
}
...
// резюмируя, следует отметить, что перекрытие - bad practise
// и обычно нужно стараться избегать его использование 
// (так как это explicit pseudopolimorphism)


// implicit shadowing
// видим, что может возникать такое непредсказуемое и опасное поведение
var o = {
  a: 1
}

var ob = Object.create(o);

o.hasOwnProperty('a'); // true
ob.hasOwnProperty('a'); // false
...
// непредсказуемо и потенциально опасно
ob.hasOwnProperty('a'); // true
